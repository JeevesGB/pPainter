...
from PyQt6.QtWidgets import (
    ...
    QVBoxLayout, QWidget, QDockWidget, QTableWidget, QTableWidgetItem, QHeaderView, QSizePolicy
)
...
class MainWindow(QMainWindow):
    ...
    def create_toolbar(self):
        ...
        brush_act.triggered.connect(lambda: self.set_tool('brush'))
        eraser_act = QAction("Eraser", self)
        eraser_act.triggered.connect(lambda: self.set_tool('eraser'))
        fill_act = QAction("Fill", self)
        fill_act.triggered.connect(lambda: self.set_tool('fill'))
        pick_act = QAction("Picker", self)
        pick_act.triggered.connect(lambda: self.set_tool('picker'))
        toolbar.addAction(brush_act)
        toolbar.addAction(eraser_act)
        toolbar.addAction(fill_act)
        toolbar.addAction(pick_act)
